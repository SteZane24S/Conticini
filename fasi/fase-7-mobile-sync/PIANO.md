...
Questa fase si apre **dopo** la fase 6: portare sul telefono un modello dati che sta per cambiare
...
- **Atomicità di dominio**: trasferimento a due gambe, stipendio con il suo ciclo, conferma di
...
### 7.1 — Protocollo, snapshot e applicazione dei pacchetti
...
- Costruzione dello snapshot dal database, riusando la logica di `packages/server/src/routes/export.ts:79-114`,
  che già esporta tutte le tabelle con i tombstone e le colonne di sincronizzazione.
...
modifica una riga cambiata nel frattempo produce un conflitto e non scrive niente; un trasferimento
arriva intero o non arriva; un pacchetto che dipende da uno non ancora presente resta in attesa e
...
- Accesso con Google, caricamento dello stato da Drive, Prospetto, saldi dei conti, elenco
  movimenti. Layout stretto da telefono.
...
- Inserimento rapido dei movimenti, con autocompletamento e categoria proposta (il calcolo è già
...
### 7.5 — In attesa, conflitti, chiusura della fase
...
i due client OAuth e passaggio in produzione (giro 7.2 per il PC, 7.3 per il telefono); account
Cloudflare e collegamento al repository privato (giro 7.3).
